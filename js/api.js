  ...
  async buscarPensamentosPorTermo(termo) {
    try {
      ...
      const pensamentosFiltrados = pensamentos.filter((pensamento) => {
        const conteudo = pensamento.conteudo || ""; // Garante que seja uma string
        const autoria = pensamento.autoria || ""; // Garante que seja uma string

        return (
          conteudo.toLowerCase().includes(termoEmMinusculas) ||
          autoria.toLowerCase().includes(termoEmMinusculas)
        );
      });

      return pensamentosFiltrados;
    } catch (error) {
      console.error("Erro ao filtrar pensamentos:", error); // Log para depuração
